...
public class Player : MonoBehaviour, ICharacter
{
    ...
    public bool _halfAttack = false;
    public bool _wasHalfAttack = false;
    public int _numberOfSkippedTurns = 0;
    public int _halfAttackTurns = 0;
    ...
    public bool wasHalfAttack
    {
        get { return _wasHalfAttack; }
        set
        {
            _wasHalfAttack = value;
        }
    }

    public int numberOfSkippedTurns
    {
        get { return _numberOfSkippedTurns; }
        set
        {
            _numberOfSkippedTurns = value;
        }
    }

    public int halfAttackTurns
    {
        get { return _halfAttackTurns; }
        set
        {
            _halfAttackTurns = value;
        }
    }
    ...
    public int Health
    {
        ...
        set
        {
            ...
            //PArea.Portrait.TakeLife(health);
            if (value <= 0)
            {
                Die();
            }
    ...
    public void GetSDLifeBonus()
    {
        int temp = this.Health * 2;
        this.Health = temp;
    }
    ...
    public void HalfAttackCardsInHand()
    {
        int index = 0;
        foreach (CardLogic card in this.hand.CardsInHand)
        {
            int temp = card.ca.Attack;
            card.ca.previousAttack = card.ca.Attack;
            if (temp == 1)
            {
                card.ca.Attack = 0;
                this.PArea.handVisual.GetCardAtIndex(index).GetComponent<OneCardManager>().cardAsset.Attack = 0;
                //this.PArea.handVisual.GetCardAtIndex(index).GetComponent<OneCreatureManager>().cardAsset.Attack = 0;
            }
            else
            {
                card.ca.Attack = temp / 2;
                this.PArea.handVisual.GetCardAtIndex(index).GetComponent<OneCardManager>().cardAsset.Attack = card.ca.Attack;
                //this.PArea.handVisual.GetCardAtIndex(index).GetComponent<OneCreatureManager>().cardAsset.Attack = card.ca.Attack;
            }
            card.ca.halvedCard = true;
            index++;
        }
        //pentru afectarea cartilor de pe masa
        //int indexTable = 0;
        //foreach (CreatureLogic creature in this.table.CreaturesOnTable)
        //{
        //    int temp = creature.ca.Attack;
        //    creature.ca.previousAttack = creature.ca.Attack;
        //    if (temp == 1)
        //    {
        //        creature.ca.Attack = 0;
        //        //this.PArea.handVisual.GetCardAtIndex(index).GetComponent<OneCardManager>().cardAsset.Attack = 0;
        //        this.PArea.tableVisual.GetCreatureAtIndex(index).GetComponent<OneCreatureManager>().cardAsset.Attack = 0;
        //    }
        //    else
        //    {
        //        creature.ca.Attack = temp / 2;
        //        //this.PArea.handVisual.GetCardAtIndex(index).GetComponent<OneCardManager>().cardAsset.Attack = creature.ca.Attack;
        //        this.PArea.tableVisual.GetCreatureAtIndex(index).GetComponent<OneCreatureManager>().cardAsset.Attack = creature.ca.Attack;
        //    }
        //    creature.ca.halvedCard = true;
        //    indexTable++;
        //}
    }

    public void RestoreAttckCardsInHand()
    {
        int index = 0;
        foreach (CardLogic card in this.hand.CardsInHand)
        {
            if (card.ca.halvedCard == true)
            {
                card.ca.Attack = card.ca.previousAttack;
                this.PArea.handVisual.GetCardAtIndex(index).GetComponent<OneCardManager>().cardAsset.Attack = card.ca.Attack;
                int cardsOnTable = this.table.CreaturesOnTable.Capacity;
                card.ca.halvedCard = false;
                index++;
            }
        }
        //pentru afectarea cartilor de pe masa
        ...
        //foreach (CreatureLogic creature in this.table.CreaturesOnTable)
        //{
        //    if (creature.ca.halvedCard == true)
        //    {
        //        creature.ca.Attack = creature.ca.previousAttack;
        //        this.PArea.tableVisual.GetCreatureAtIndex(indexTable).GetComponent<OneCreatureManager>().cardAsset.Attack = creature.ca.Attack;
        //        int cardsOnTable = this.table.CreaturesOnTable.Capacity;
        //        creature.ca.halvedCard = false;
        //        index++;
        //    }
        //}
    }
