...
public class TurnManager : MonoBehaviour
{
    ...
    public Player whoseTurn
    {
        ...
        {
            _whoseTurn = value;
            BeginTurn(_whoseTurn.skipTurn, _whoseTurn.halfAttack, _whoseTurn.numberOfSkippedTurns, _whoseTurn.halfAttackTurns);
        }
    }

    private void BeginTurn(bool skipTurn, bool halfAttack, int numberOfSkippedTurns, int halfAttackTurns)
    {
        timer.StartTimer();
        ...
        if (skipTurn)
        {
            whoseTurn.skipTurn = false;
            whoseTurn.numberOfSkippedTurns--;
            whoseTurn = whoseTurn.otherPlayer;
        }
        if (halfAttackTurns == 0)
        {
            whoseTurn.halfAttack = false;
            whoseTurn.RestoreAttckCardsInHand();
        }
        else if (halfAttack)
        {
            whoseTurn.halfAttackTurns--;
        }
        // remove highlights for opponent.
        whoseTurn.otherPlayer.HighlightPlayableCards(false);
    }
